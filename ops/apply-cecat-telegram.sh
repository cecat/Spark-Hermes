#!/usr/bin/env bash
# Write cecat's channels.telegram block + binding into her sandbox openclaw.json.
#
# Direct analogue of ops/apply-cecat-slack.sh — same shape, same container
# discovery, same embedded-python patch, same SIGTERM restart. Only the channel
# and its credential differ. Idempotent: re-running overwrites the telegram
# block and the telegram binding, leaving everything else in the config
# untouched.
#
...
#
# Token comes from ~/.openclaw-secrets/cecat-telegram.env (mode 600, host-side):
...
#
# Egress is a SEPARATE prerequisite, not done here: cecat reaches Telegram as
# /usr/local/bin/node, so bringup/50-openshell-policies/cecat-telegram-egress.yaml
# must already be on her sandbox or the adapter is denied at the L7 proxy.
# Apply it with:
...
#
# Run: bash ops/apply-cecat-telegram.sh
set -eu
